...
enum Commands {
    ...
    GenerateKey {
        ...
        /// Optional output file to save all key information
        #[arg(short, long)]
...
async fn main() -> Result<()> {
    ...
    match args.command {
        Commands::GenerateKey { name, count, output } => {
            // Create a vector to store key information if output is specified
            let mut key_info = Vec::new();
            
            if count == 1 {
                generate_key_pair(&name)?;
                if let Some(output_path) = &output {
                    // Get the key info after generation
                    let key_store = load_key_store()?;
                    if let Some(key_pair) = key_store.keys.get(&name) {
                        key_info.push(format!("Key: {}\nPublic Key: {}", name, key_pair.public_key_string));
                    }
                }
            } else {
                for i in 1..=count {
                    let key_name = format!("{}{}", name, i);
                    println!("\nGenerating key {}/{}: {}", i, count, key_name);
                    generate_key_pair(&key_name)?;
                    
                    if let Some(output_path) = &output {
                        // Get the key info after generation
                        let key_store = load_key_store()?;
                        if let Some(key_pair) = key_store.keys.get(&key_name) {
                            key_info.push(format!("Key: {}\nPublic Key: {}", key_name, key_pair.public_key_string));
                        }
                    }
                }
            }
            
            // Save key information to file if output is specified
            if let Some(output_path) = output {
                let output_content = key_info.join("\n\n");
                fs::write(&output_path, output_content)
                    .with_context(|| format!("Failed to write key information to {}", output_path.display()))?;
                println!("\n✅ Key information saved to {}", output_path.display());
            }
        },
        Commands::ListKeys => {
            list_keys()?;
